...
转载自：[https://github.com/unknwon/the-way-to-go\_ZH\_CN/blob/master/eBook/14.2.md](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/14.2.md) by unknown
...
`int2 = <- ch` 表示：变量 int2 从通道 ch接收数据；假设 int2 已经声明过了，如果没有的话可以写成：`int2 := <- ch`。
...
为了知道计算何时完成，可以通过通道回报。在例子 `go sum(bigArray)` 中，要这样写：
